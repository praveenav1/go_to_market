...
def admin_login():
    try:
        ...
        # First try JSON-backed users (existing fallback)
        ...
        # If not found in JSON or hash mismatch, try MySQL Users table (plaintext Password column)
        try:
        ...
        try:
            with conn.cursor() as cursor:
                ...
                # Compare plaintext password (existing Users table stores `Password`)
                ...
                # Fetch teams where this user is an approver (optional)
                cursor.execute(
...
@app.route('/api/team_requests', methods=['POST'])
def create_team_request():
    try:
        ...
        print(f"Error creating team request: {str(e)}")
        return jsonify({'error': 'Failed to create request'}), 500


@app.route('/api/team_requests', methods=['GET'])
def list_team_requests():
    try:
        ...
        print(f"Error listing team requests: {str(e)}")
        return jsonify({'error': 'Failed to list requests'}), 500


@app.route('/api/team_requests/<int:request_id>/approve', methods=['POST'])
def approve_team_request(request_id):
    try:
        ...
        print(f"Error approving team request: {str(e)}")
        return jsonify({'error': 'Failed to approve request'}), 500


@app.route('/api/team_requests/<int:request_id>/reject', methods=['POST'])
def reject_team_request(request_id):
    try:
        ...
        print(f"Error rejecting team request: {str(e)}")
